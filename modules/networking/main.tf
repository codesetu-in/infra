...
resource "aws_subnet" "database" {
  ...
  tags = { Name = "${local.name_prefix}-database-${var.azs[count.index]}" }
}
...
resource "aws_vpc_endpoint" "cloudwatch_logs" {
  ...
  tags = { Name = "${local.name_prefix}-cwlogs-endpoint" }
}
...
resource "aws_security_group" "ecs_tasks" {
  ...
  egress {
    description = "HTTPS for AWS APIs (ECR, CloudWatch, Secrets Manager)"
    from_port   = 443
    to_port     = 443
    protocol    = "tcp"
    cidr_blocks = ["0.0.0.0/0"]
  ...
  egress {
    description     = "PostgreSQL to database layer"
    from_port       = 5432
    to_port         = 5432
    protocol        = "tcp"
...
resource "aws_security_group" "redis" {
  ...
  description = "ElastiCache Redis: accept connections from ECS tasks only"
  vpc_id      = aws_vpc.main.id
